  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Power calculator - Album Art Matrix" },
      {
        ...
        content:
          "Model trunk IR drop, panel voltage and supply trim for a 3x3 HUB75 P2.5 LED wall.",
      },
    ],
  }),
  component: PowerPage,
...
function FeedDiagram({ i, o }: { i: PowerInputs; o: PowerOutputs }) {
  ...
  const vs = [
    i.supplyVolts,
    i.supplyVolts - o.trunkDrop,
    i.supplyVolts - o.trunkDrop - o.fuseDrop,
    o.panelVolts,
  ];
  ...
  segments.push({
    x0: x,
    x1: (x += seg[0]),
    label: `trunk ${i.trunkLengthM.toFixed(1)} m / ${i.trunkAwg} awg`,
    drop: o.trunkDrop,
    stroke: trunkW,
  });
  const fuseX = x + 4;
  ...
  segments.push({
    x0: x,
    x1: wireX1,
    label: `harness ${i.harnessLengthM.toFixed(1)} m / 18 awg`,
    drop: o.harnessDrop,
    stroke: 2,
  });
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full">
      ...
      <text
        ...
        style={{ font: "9px var(--font-mono)", letterSpacing: "0.1em" }}
      >
        PSU TRIM
      </text>
      <text
      ...
      {segments.map((s, idx) => (
        <g key={idx}>
          ...
          <text
            ...
            textAnchor="middle"
            fill="var(--muted-foreground)"
            style={{ font: "8.5px var(--font-mono)", letterSpacing: "0.06em" }}
          >
            {s.label}
          </text>
          <text
            ...
            y={railY + 18}
            textAnchor="middle"
            fill={s.drop > 0.15 ? "var(--warning)" : "var(--muted-foreground)"}
      ...
      <text
        ...
        y={H - 48}
        fill="var(--muted-foreground)"
        style={{ font: "8px var(--font-mono)", letterSpacing: "0.1em" }}
      >
    ...
    <div className="space-y-6">
      <header className="enter space-y-1.5">
        ...
        <p className="max-w-xl text-xs leading-relaxed text-muted-foreground">
          Planning tool for the wiring: what each section of the feed costs in volts, and where the
          supply has to sit so 5.00 V actually arrives at the panel.
        </p>
      </header>
      ...
      <div className="placard enter-1 p-4">
        ...
        {out.warning && (
          <p className="num mt-2 border-t border-primary/40 pt-2.5 text-[10px] leading-relaxed text-primary">
            {out.panelVolts.toFixed(2)} V at the panel is below 4.90 V. The driver chips misbehave
            under that: dim patches, colour shifts, flicker on bright frames - which looks exactly
            like a software bug. Shorten the trunk, go heavier, split the feed, or trim the supply
            up.
          </p>
        )}
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
          ...
          <div className="grid gap-3.5 sm:grid-cols-2">
            ...
            <Field label="panels per feed section">
              <Input
                ...
                min={1}
                onChange={(e) =>
                  set({ panelsPerSection: Math.max(1, Number(e.target.value) || 1) })
                }
              />
            </Field>
        ...
        <div className="space-y-4">
          <div className="placard p-5">
            ...
            <Readout
              label="trunk resistance, out + back"
              value={`${(out.trunkOhms * 1000).toFixed(1)} mΩ`}
            />
            ...
            <Readout
              label="voltage at the panel"
              value={`${out.panelVolts.toFixed(2)} V`}
              tone={out.warning ? "alert" : undefined}
            />
          ...
          <HonestNote>
            Resistances are copper at 20 C (14 AWG 8.286 mΩ/m, 18 AWG 20.95 mΩ/m per conductor).
            Real connectors, crimps and hot copper all add drop. Treat these numbers as a model to
            plan with, then measure at the panel.
          </HonestNote>
        </div>
      </div>
    </div>
