  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Now-playing sources - Album Art Matrix" },
      {
        ...
        content:
          "Reorder, enable and configure the now-playing source chain. No source ever invents a track.",
      },
    ],
  }),
      ...
      <header className="enter flex flex-wrap items-end justify-between gap-3">
        <div className="min-w-0 space-y-1.5">
          ...
          <p className="max-w-xl text-xs leading-relaxed text-muted-foreground">
            The first source that returns an answer wins. A source that errors is logged and
            skipped, never fatal, and never invents a track.
          </p>
        </div>
            ...
            <Card key={id} className={answered ? "border-[var(--art)]" : undefined}>
              <CardHeader className="gap-2">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="num text-lg text-muted-foreground/50">
                ...
                {status.lastAnswerTier && (
                  <p className="num text-[11px] text-muted-foreground">
                    answered by: {status.lastAnswerTier}
                  </p>
                )}
              </CardHeader>
...
function SourceSettings({ id }: { id: SourceId }) {
  ...
  const cfg = sourceConfigs[id];
  const setCfg = (patch: Record<string, string | number | boolean>) =>
    setSourceConfig(id, { config: patch });
      ...
      <div className="space-y-3">
        ...
        {mode === "direct" ? (
          <HonestNote tone="warn">
            Direct mode drives AppleScript and MusicKit on macOS directly. A browser cannot reach
            either, so it is unavailable here. It is a real option in the desktop build - it is not
            hidden and it is not faked.
          </HonestNote>
        ) : (
          <>
            <Field
            ...
            <HonestNote>
              Tiers, in order: companion state file → Music.app over AppleScript (real-time local
              playback including AirPlay from the Mac, never launches Music) → MusicKit account
              query (most recently played across all devices, which is what catches an iPhone).
              Artwork is always URL-based so it caches by URL. If the browser blocks the response,
              the local server must send an <code>Access-Control-Allow-Origin</code> header.
            </HonestNote>
          </>
        )}
      </div>
      ...
      <div className="space-y-3">
        <div className="grid gap-3 sm:grid-cols-3">
          <Field label="AcoustID API key">
            <Input
              className="num"
              value={String(cfg.config.apiKey ?? "")}
        ...
        <Field
          label="Fingerprint helper URL"
          hint="A local helper that captures the input, runs Chromaprint (fpcalc) and serves GET /fingerprint."
        >
          <Input
            className="num"
            value={String(cfg.config.bridgeUrl ?? "")}
            onChange={(e) => setCfg({ bridgeUrl: e.target.value })}
            placeholder="http://localhost:8788/fingerprint"
          />
        </Field>
        <HonestNote tone="warn">
          Waiting on: Chromaprint fingerprinting, which a browser cannot do natively. Until the
          helper is configured and reachable, this source stays in needs-bridge and returns nothing.
          It will never fabricate a match. The chain after that is AcoustID → MusicBrainz recording
          → Cover Art Archive artwork, free and up to 1200x1200.
        </HonestNote>
      </div>
    );
  }
    ...
    return (
      <div className="grid gap-3 sm:grid-cols-2">
        <Field label="Last.fm username">
          <Input
            className="num"
            value={String(cfg.config.username ?? "")}
            onChange={(e) => setCfg({ username: e.target.value.trim() })}
          />
        </Field>
  ...
  if (id === "mpris") {
    return (
      <div className="space-y-3">
        <Field
          label="playerctl helper URL"
          hint="A local helper that queries MPRIS over D-Bus and serves GET /mpris."
        >
          <Input
            className="num"
            value={String(cfg.config.bridgeUrl ?? "")}
            onChange={(e) => setCfg({ bridgeUrl: e.target.value })}
            placeholder="http://localhost:8789/mpris"
          />
        </Field>
...
function DemoPanel() {
  ...
  return (
    <div className="space-y-3">
      <HonestNote>
        Demo content. These covers are generated in the app; they are not a real now-playing
        reading.
      </HonestNote>
      <div className="flex flex-wrap gap-2">
