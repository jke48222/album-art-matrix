...
// What pipeline.worker.ts posts back. The typed arrays cross as plain
// ArrayBuffers, so deserialise() puts them back into views.
type SerialisedStep = Omit<PipelineResult["steps"][number], "buffer"> & { buffer: ArrayBuffer };
type SerialisedResult = Omit<PipelineResult, "balanced" | "unbalanced" | "steps"> & {
  balanced: ArrayBuffer;
  unbalanced: ArrayBuffer;
  steps: SerialisedStep[];
};
type WorkerReply = { id: number; ok: boolean; result: SerialisedResult; error?: string };
...
export function usePipelineWorker() {
  ...
  const run = useCallback(
    async (img: RgbaImage, params: PipelineParams): Promise<PipelineResult> => {
      setBusy(true);
      try {
...
function deserialise(result: SerialisedResult): PipelineResult {
  return {
    ...result,
    balanced: new Uint8Array(result.balanced),
    unbalanced: new Uint8Array(result.unbalanced),
    steps: result.steps.map((s) => ({ ...s, buffer: new Uint8Array(s.buffer) })),
  };
}
