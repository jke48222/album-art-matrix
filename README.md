...
| The wall's own ears, the voice (`brain/voice/`) | Built 2026-09-15. A wake word on the ear's stream (openWakeWord, "hey Jarvis" until a "hey wall" head is trained), the Horizon face while it listens, speech to text on the Pi (faster-whisper tiny, about a second and a half a sentence), a grammar of commands done with no cloud, and Claude for the rest. Three Voice knobs; `GET /voice`, `POST /voice/wake`, `POST /voice/say` for testing without a microphone. Twenty-nine offline tests across the grammar, the faces and the state machine. |
...
| Weather faces (`brain/weather.py`, `brain/art/weather.py`) | Built 2026-09-15 and proven on the Pi with the real forecast for the room's place. A face that is the weather: the sun on its real arc by day, the moon with tonight's phase by night, clouds that drift with the wind, rain that falls with the forecast's density and leans, snow that settles on the bottom edge, fog, thunder; the temperature in 2x digits at 64, 4x at 192 with the high and low and the next six hours. Every WMO code has a scene. Open-Meteo, no key; a place named once on the phone's Weather page; Fahrenheit or Celsius; a weather choice under Nothing playing. Seven tests. |
...
| Imagine (`brain/imagine.py`) | Built 2026-09-15, on the Pi, waiting for an image key (Services > Images: OpenAI gpt-image-1 or Google Imagen 4, the choice on the phone). "Create a purple elephant", by voice or from the Imagine page: Claude rewrites the words as a prompt for a panel this size (one bold centred subject, flat colours, no text; a fixed template when there is no Claude key), the image model draws it, the picture goes through the sleeve pipeline at the wall's size and stays in the frame face for ten minutes. Kept at full size with its words in `imagined/`, two hundred at most; `GET /imagine` lists them, `GET /imagine/<id>.png` serves one, `POST /imagine/show` puts one back. One every ten seconds, every one costed. Four tests, both providers' requests checked, the picture at 64 and 192. |
| AirPlay to the wall (`brain/nowplaying/airplay.py`, `docs/AIRPLAY.md`) | Brain side built 2026-09-15 and proven on the Pi with a recorded stream written into the pipe; waiting for `sudo apt install shairport-sync` and the config in the guide (Debian's 4.3.7 is an AirPlay 2 build, so nqptp from source too). A reader on shairport-sync's metadata pipe: title, artist, album, length, the artwork bytes, the position as RTP timestamps advanced by the clock, pause, resume and end, the sender's name. Answers right after the phone in the chain, with the artwork served at `/art/airplay/<key>.jpg` for the phone and handed to the brain's own fetcher without a round trip; the track id changes when the art lands so the sleeve goes up the moment it arrives. AirPlay's state on the Services page. Four tests over a recorded stream at every chunking. |
