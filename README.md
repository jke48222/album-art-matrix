...
| Discogs shelf (`brain/shelf.py`) | Built on `codex/build`: resumable collection sync, 40-release matcher fixture, pressing details, play counts, marketplace cache and ownership glyph. Both glyph sizes rendered; live token and physical phone acceptance pending. [SHELF.md](docs/SHELF.md). |
| Earworm and discovery (`brain/earworm.py`, `brain/discover.py`) | Built on `codex/build`: structured lyric recall, catalogue sleeves, low-confidence pair, ten-minute Show and yt-dlp Play. Four layouts inspected; live Claude and phone acceptance pending. [DISCOVER.md](docs/DISCOVER.md). |
| AirPlay receiver (`brain/nowplaying/airplay.py`) | Built and launched on the Codex Pi: silent AirPlay 2 receiver, metadata pipe, cover endpoint and Services status. Feature-off cleanup verified; live Apple sender acceptance pending. [AIRPLAY.md](docs/AIRPLAY.md). |
| Leave a note (`brain/note.py`) | Built on `codex/build`: timed ticker with previous-face return and control cancellation; signed Tell the wall Shortcut. Both sizes rendered; Shortcut import on iPhone pending. [ASK.md](docs/ASK.md#leave-a-note). |
| Wake and speech (`brain/voice.py`) | Built on `codex/build`: local Jarvis detection, Whisper, safe short-command matching, Hearing controls. Pi synthetic phrase and 1.69 s transcription verified; custom Hey wall and room acceptance pending. [VOICE.md](docs/VOICE.md). |
| Horizon (`brain/art/horizon.py`) | Built on `codex/build`: 250 ms collapse, voice line, thinking bead, 400 ms reveal and missed-speech return. Both layouts rendered and inspected. [HORIZON.md](docs/HORIZON.md). |
| Ask the wall (`brain/ask.py`) | Built on `codex/build`: Claude tools, temporary answer pages, Services setup and signed Siri Shortcut. Both geometries rendered and inspected; offline timeout and tool tests pass. Live Claude response requires the owner key. [ASK.md](docs/ASK.md). |
| Taught songs (`brain/teach.py`) | Built on `codex/build`: native Olaf before Shazam, preview and room learning, fingerprint-only storage, Hearing library management. Six offline tests; MALI preview recognized on Pi in 20 ms. [TEACH.md](docs/TEACH.md). Physical-room acceptance pending. |
| Knock twice (`brain/gestures.py`) | Built on `codex/build`: double-knock and directional-whistle detectors, Hearing knobs, persistent return face. 11 synthetic tests pass; real room acceptance pending. [KNOCK.md](docs/KNOCK.md). |
| Vinyl scrobbling (`brain/scrobble.py`) | Built on `codex/build`: ear-only hearing thresholds, persistent dedupe, seven-day retry queue, token and status in Tessera. 23 offline tests pass. See [SCROBBLE.md](docs/SCROBBLE.md). Live acceptance requires the owner token. |
