# Three objects, from what you liked

You picked six things across two studies: ChatGPT's **Index** (the aluminium
cassette, art edge to edge, an honest 118 mm side) and **Soft Monument** (the
octagonal textile relief with its black power shoe); and from here **Console**
(charcoal wool, a machined bezel), **Vitrine** (the picture down a cloth-lined
well), the **boxed** buildable design (matte black, fully enclosed, vented) and
the **walnut** shadow box (black anodised reveal, brass keyed mitres).

Read together they say one thing four ways: honest depth, a precise metal
edge, a warm material against black, and nothing that is not the picture. The
first two objects below are those six resolved into one; the third is new.

```
/Applications/Blender.app/Contents/MacOS/Blender --background \
    --python design/alt_model.py -- --design strap --out design/renders \
    --face design/face192.png
```

`--design case|hearth|strap`. No knob, no tenth panel, in any of them. The
C14 with its switch is placed where a hand goes, differently in each.

| | Envelope | Depth | Materials | The switch |
|---|---|---|---|---|
...
Index's cassette with Console's face. Four satin aluminium sides, the honest
depth Index argued for, and a charcoal wool front plate instead of bare LEDs:
the picture sits 15 mm back in a black anodised well (a shallow Vitrine),
behind smoked acrylic, inside the black anodised reveal from the walnut design
standing 3 mm proud of the glass the way Console's bezel did. Fully enclosed
and vented, from the boxed build. The mains module is on the underside at the
right, 165 mm off centre, where Index put it, so you reach under with your
right hand and the cord drops down the side.

What it takes from each: Index's silhouette and side; Console's cloth and
proud bezel; Vitrine's recess, made shallow; Boxed's enclosure; Walnut's
anodised black at the opening. What it leaves out: anything warm. This is the
one that belongs next to the brushed-metal lamp and the white turntable.

## 2. Hearth — the warm one

Soft's broken rectangle tamed to a square with 70 mm chamfers, in the walnut
design's veneer with its brass keys moved from the mitres to the chamfers,
where they read as four short bright lines when you pass. The picture is 40 mm
down a well lined in Vitrine's oatmeal wool, behind smoked acrylic in a brass
bezel: a reliquary in a wooden case. Soft's black power shoe hangs from the
bottom edge 140 mm left of centre, the one precise interruption in a soft
form, and the switch faces the floor inside it.

What it takes: Soft's silhouette and shoe; Walnut's veneer, brass and depth;
Vitrine's lined well; Console's bezel, in brass. It is the deepest of the
three at 158 mm and the most furniture-like; it is also the one that most
...
| Part | For | About |
|---|---|---|
| Aluminium sheet or angle for four sides | Case | $37 |
